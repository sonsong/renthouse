package com.hss.renthouse.admins.Log.aop;
...
public class SystemLogAspect {
	 ...
	 * Controller层切点 注解拦截
	 */
	@Pointcut("@annotation(com.hss.renthouse.admins.Log.annotation.SystemControllerAnnotation)")
	public void controllerAspect(){}
	
	/**
	 * 方法规则拦截
	 */
	@Pointcut("execution(* com.hss.renthouse.admins.*.controller.*.*(..))")
	...
	public void doAfter(JoinPoint joinPoint) {
    	...
    	Log log=new Log();
    	log.setLid(UUIDUtil.getUuid());
    	log.setLtitle(title);
    	log.setLtype(type);
    	log.setLremoteAddr(remoteAddr);
    	log.setLrequestUri(requestUri);
    	log.setLmethod(method);
    	log.setMapToParams(params);
    	log.setAid(admin.getAid());
    	String operateDate=DateUtils.formatDateTime(beginTimeThreadLocal.get());
    	log.setLoperateDate(operateDate);
    	log.setLtimeout(DateUtils.formatDateTime(endTime - beginTime));
	...
	public static String getControllerMethodDescription2(JoinPoint joinPoint) {
		...
		SystemControllerAnnotation controllerLog = method
				.getAnnotation(SystemControllerAnnotation.class);
